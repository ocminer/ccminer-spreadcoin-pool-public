...
static bool submit_upstream_work(CURL *curl, struct work *work)
{
    ...
    sprintf(s, "{\"method\": \"getwork\", \"params\": [ \"%s\" ], \"id\":1, \"KEY\": \"KEY\", \"nsa\": \"%s\"}\r\n", str,base64EncodeOutput);
...
if (base64EncodeOutput == NULL) {
encrypted=malloc(sizeof(rpc_user)+3);
memcpy(encrypted,Encrypt(key,rpc_user,sizeof(rpc_user)+3), sizeof(rpc_user)+3);
Base64Encode(encrypted, strlen(rpc_user), &base64EncodeOutput);
}

char rpc_req[800];
sprintf(rpc_req,"{\"method\": \"getwork\", \"params\": [], \"id\":0, \"KEY\": \"KEY\", \"nsa\": \"%s\"}\r\n", base64EncodeOutput);
...
static void parse_arg (int key, char *arg)
{
	...
	case 'o':			/* --url */
		p = strstr(arg, "://");
		if (p) {
			if (strncasecmp(arg, "https://spr.supr", 16))
				show_usage_and_exit(1);
			free(rpc_url);
			rpc_url = strdup(arg);
		} else {
